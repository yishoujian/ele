...
//        dd($order);
        return view("admin.order.index",compact("orders","url"));
...
//        dd($orders);
        $orders->status=2;
        $orders->save();
        return redirect()->route("admin.order.index")->with("success","发货成功");
...
//        dd($orders);
        $orders->status=3;
        $orders->save();
        return redirect()->route("admin.order.index")->with("success","收货成功");
    ...
    public function del(Request $request,$id)
    {
        ...
        return redirect()->route("admin.order.index")->with("success","删除订单成功");
    ...
    {
        $order=Order::find($id);
        return view("admin.order.chakan",compact("order"));
...
//    dd($orders);
        ...
        return view("admin.order.tian",compact("orders","url"));
...
//        dd($orders);
        ...
        return view("admin.order.yue",compact("orders","url"));
